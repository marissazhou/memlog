...
def square_root(values):
	'''This returns the root value of squres of elements in values, values can be two or more
	
	:param values: the list of numbers that need to get a square root 
	:type values: list of numeric 
	:returns: square root of all values in the list 
	'''
	square = 0
	root = values[0]
	if len(values) < 2:
...
def get_median(values):
	"""This returns the median value of all values in the list values
	
	:param values: the list of numbers that need to get a median from
	:type values: list of numeric 
	:returns: median of all values in the list 
	"""
	median = 0
	length = len(values)
